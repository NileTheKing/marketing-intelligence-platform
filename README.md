...
| 모듈 | 역할 | 주요 기술 |
|--------|----------------|------------|
| **`entry-service`** | 트래픽 게이트웨이, FCFS 진입 제어, 행동 로그 수집 | Spring Boot, Redis, Kafka |
| **`core-service`** | 비즈니스 로직 처리, 데이터 영속화, 통계 분석 | Java 21, JPA, Redisson, Spring Batch |
| **`common-messaging`** | 공통 DTO, Kafka 토픽 정의, 도메인 이벤트 | Java Library |
| **`axon-tracker`** | 경량 사용자 행동 추적 JS SDK | Vanilla JS (< 5KB) |
...
**AI**:
- Gemini 2.0 Flash (Function Calling 기반 RAG 구현)
