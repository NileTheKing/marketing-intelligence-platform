...
- `claim`은 `analysisClaimToken`과 짧은 분석 점유 만료 시각을 발급한다. FastAPI 장애 뒤에는 만료된 case를 다시 분석할 수 있다. 자동 worker는 `PENDING`과 점유 만료 case만 가져가며, `ANALYSIS_FAILED` 또는 운영자가 직접 선택한 점유 만료 case는 Slack의 추가 확인·확인 결과 입력에서만 다시 claim한다.
